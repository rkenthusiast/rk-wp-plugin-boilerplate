...
use rk_wp_plugin_boilerplate\Engine;

/**
 * rk_wp_plugin_boilerplate Initializer
 */
class Initialize {
	...
	private function load_classes() {
		$this->classes = \apply_filters( 'rk_wp_plugin_boilerplate_classes_to_execute', $this->classes );

		foreach ( $this->classes as $class ) {
			try {
				$this->initialize_plugin_class( $class );
			} catch ( \Throwable $err ) {
				\do_action( 'rk_wp_plugin_boilerplate_initialize_failed', $err );
	...
	private function initialize_plugin_class( $classtovalidate ) {
		...
		\add_filter(
			'rk_wp_plugin_boilerplate_instance_' . $classtovalidate,
			function() use ( $temp ) {
				return $temp;
			}
		);
	...
	private function get_classes( string $namespacetofind ) {
		...
		$namespacetofind = 'rk_wp_plugin_boilerplate\\' . $namespacetofind;

		// In case composer has autoload optimized
		if ( isset( $classmap[ 'rk_wp_plugin_boilerplate\\Engine\\Initialize' ] ) ) {
		...
		if ( isset( $prefix[ $namespacetofind ] ) ) {
			...
			if ( !WP_DEBUG ) {
				\wp_die( \esc_html__( 'rk-wp-plugin-boilerplate is on production environment with missing `composer dumpautoload -o` that will improve the performance on autoloading itself.', R_TEXTDOMAIN ) );
			}
