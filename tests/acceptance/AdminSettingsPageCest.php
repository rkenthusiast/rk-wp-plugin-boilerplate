...
class AdminSettingsPageCest {
	...
	function add_plugin_admin_menu(AcceptanceTester $I) {
		...
		$I->see('rk-wp-plugin-boilerplate Settings', 'h2');
	}

	function add_action_link(AcceptanceTester $I) {
		$I->wantTo('check plugin list page if include mine');
		$I->amOnPluginsPage();
		$I->see('rk-wp-plugin-boilerplate', 'tr.active[data-plugin="rk-wp-plugin-boilerplate/rk-wp-plugin-boilerplate.php"] td strong');
	}
