...
class ActDeact extends Base {
	...
	public static function upgrade_procedure() {
		...
		$version = \strval( \get_option( 'rk-wp-plugin-boilerplate-version' ) );

		if ( !\version_compare( R_VERSION, $version, '>' ) ) {
			return;
		}

		\update_option( 'rk-wp-plugin-boilerplate-version', R_VERSION );
		\delete_option( R_TEXTDOMAIN . '_fake-meta' );
	}
